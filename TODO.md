...
| 44c | **DISTRIBUTION DES AGENTS — faire trouver le MCP `/agents/mcp`** | **PRIORITÉ N°1 — ✅ L'ESSENTIEL FAIT (25 juin)** | Serveur MCP distant LIVE. **Surfaces auto-descriptives ✅ déployées prod** : `/llms.txt`, `/for-agents` (bilingue), `.well-known/mcp.json`, `ai-plugin.json` `x_mcp`. **✅ REGISTRE OFFICIEL MCP PUBLIÉ** : `io.github.NeousAxis/aya-registry` v1.0.0, **status active** (via `mcp-publisher` + login github device-flow) → **Glama + PulseMCP l'ingèrent automatiquement** (confirmé sur leur page : « ingest from the Official MCP Registry daily »). **✅ awesome-mcp PR #8751** ouverte. **✅ mcp.so** formulaire pré-rempli (reste sign-in). **⏳ RESTE CLIC CYRIL** : Smithery + mcp.so = créer compte (GitHub OAuth, prohibé pour moi) ; **broadcast** = posts prêts dans `POLLEN-AGENTS-BROADCAST.md` mais à tirer délibérément (compte X connecté = @WeatherAgentApp ≠ marque AYA ; Reddit/HN/PH = lancements one-shot timing-sensibles). |
| 44d | **Côté DEMANDE — activer les premiers opérateurs d'agents** | Critique | Le vrai cold-start. Boucle prouvée + MCP distant prêt. Brancher 1-2 agents réels (dont **Swiss Agents** de Cyril → lui donner l'URL MCP `https://ai-visionary.xyz/agents/mcp`, zéro code) qui font des vraies requêtes + cashback. + Awin dès clés Cyril pour l'inventaire marques connues. |
| 44e | **AYO→AYA — ingestion depuis l'usage (flywheel)** | ✅ **FAIT + vérifié prod 28 juin** | Tout diagnostic AYO **et** toute recherche d'agent (outil MCP `index_company` + auto-seed `get_company_details`) inscrit l'entreprise dans AYA (entrée indexée `AYO-SCAN`, dédup par URL, scan AYO en tâche de fond `next/after`). Endpoint `app/api/aya/index-company`. Commit `1c607c02`. Voir STATE.md §Agents + mémoire `project_diagnostic_auto_index_aya`. ⚠️ surveiller coût scan (mitigé dédup-first) + spam domaines bidon (réversible `DELETE WHERE data_origin='AYO-SCAN'`). |
