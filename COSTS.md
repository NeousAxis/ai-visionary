...
9. **Quand on cleanup une fuite via `git-filter-repo`** : ne jamais oublier `git push --force --tags origin` en plus du push branches, sinon les tags continuent de pointer sur les anciens commits avec la clé. Vérifier ensuite par re-clone : `git clone <url> /tmp/check && cd /tmp/check && git log --all --tags -p | grep -c "<clé>"` doit donner 0.
10. **Auditer TOUS les repos publics** (pas seulement le repo principal) avant de considérer une clé comme révoquée. Incident 10 mai : 2e fuite trouvée dans `Wise-Weather-App` après que Better-ESG était déjà suspendu pour la 1ère.
