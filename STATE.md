...
- **DISTRIBUTION (priorité n°1) — surfaces LIVE en prod (25 juin), soumissions registres = gated Cyril** :
  ...
  - ✅ **Manifestes registres prêts (repo)** : `server.json` (schéma registre MCP officiel, remote streamable-http, namespace `xyz.ai-visionary/aya-registry`), `smithery.yaml`.
  - ✅ **Runbooks rédigés** : `MCP-DISTRIBUTION-RUNBOOK.md` (étapes par registre + ce qui demande un clic Cyril) + `POLLEN-AGENTS-BROADCAST.md` (posts prêts : awesome-mcp PR, r/AI_Agents, X, Show HN, Product Hunt, Discord MCP).
  - ⏳ **CLIC CYRIL** : publier sur registre officiel MCP (`mcp-publisher` + DNS TXT ou OAuth GitHub), Smithery, mcp.so, PulseMCP, awesome-mcp PR ; puis broadcast. Voir runbook §1-2.
