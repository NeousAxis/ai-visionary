...
- **DISTRIBUTION (priorité n°1) — surfaces LIVE en prod (25 juin), soumissions registres = gated Cyril** :
  - ✅ **Surfaces auto-descriptives déployées + vérifiées prod** : `https://ai-visionary.xyz/llms.txt` (headline MCP+cashback, text/plain), page **`/for-agents`** bilingue FR/EN (config copier-coller Claude/Cursor/CLI, 5 outils, économie cashback, lien footer + sitemap), `.well-known/mcp.json` (discovery, 5 tools), `.well-known/ai-plugin.json` enrichi (`x_mcp`).
  - ✅ **REGISTRE OFFICIEL MCP PUBLIÉ (25 juin)** : `io.github.NeousAxis/aya-registry` v1.0.0, **status active**, remote `https://ai-visionary.xyz/agents/mcp` (via binaire `mcp-publisher` v1.7.9 + `login github` device-flow + `publish` ; namespace GitHub, casse exacte `NeousAxis`). `server.json` du repo = manifeste réellement publié. **→ Glama + PulseMCP ingèrent automatiquement** depuis ce registre (confirmé sur la page submit PulseMCP : « ingest from the Official MCP Registry daily, process weekly »).
  - ✅ **awesome-mcp-servers** : PR [#8751](https://github.com/punkpeye/awesome-mcp-servers/pull/8751) ouverte (Search & Data Extraction).
  - ✅ **mcp.so** : formulaire pré-rempli (reste le sign-in). **Smithery + mcp.so** soumission gatée derrière création de compte GitHub OAuth (prohibé en autonomie) → 1 clic Cyril chacun.
  - ✅ **Runbooks** : `MCP-DISTRIBUTION-RUNBOOK.md` + `POLLEN-AGENTS-BROADCAST.md`.
  - ⏳ **Broadcast (Cyril)** : posts prêts mais à tirer délibérément — le compte X connecté est `@WeatherAgentApp` (≠ marque AYA), et Reddit/HN/Product Hunt sont des lancements one-shot sensibles au timing.
