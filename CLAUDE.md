...
> Derniere mise a jour : **28 juin 2026** — **AYA grossit toute seule depuis l'usage (AYO→AYA)** : (1) **tout diagnostic AYO inscrit l'entreprise dans AYA** — entrée INDEXÉE `data_origin='AYO-SCAN'`, `payment_completed=false`, dédup par URL (jamais d'écrasement d'une entité bot/certifiée), pays via ccTLD + score plafonné 50 sans ASR (doctrine). `app/api/diagnostic/scan/route.ts` appelle `indexEntityFromDiagnostic()` (`lib/aya/registry.ts`). (2) **Les agents alimentent AYA** : 6e outil MCP `index_company(domain)` + **auto-seed** sur `get_company_details` (404 → indexe en tâche de fond via `next/after`) ; endpoint `app/api/aya/index-company/route.ts` (dédup-first → renvoie `exists` sans rescanner, sinon scan AYO async). Une recherche d'utilisateur qui croise une entreprise inconnue la fait entrer au registre = **flywheel d'usage**. À donner à un opérateur d'agent (Swiss Agents) : juste l'URL MCP `https://ai-visionary.xyz/agents/mcp` (zéro code) + la règle « cherche AYA d'abord, sinon index_company, get_cashback_offer avant de recommander ». (3) **Fix latent** : `db.getAyaEntityByUrl` ratait le shortcircuit Postgres-local sur le VPS (tapait Supabase frozen) → dédup cassée pour `registerOrUpdateEntity` + `is_aya_registered` de l'orchestrateur ; routé via `localPgGetEntityByDomain`. Déployé + vérifié end-to-end VPS (create / dédup self / dédup vs bot / MCP tools/list expose `index_company`). Réversible : `DELETE WHERE data_origin='AYO-SCAN'`. Commit `1c607c02` (branche `feature/pollen-agents`, pushé). Voir [[project_diagnostic_auto_index_aya]] + STATE.md §Agents.
>
> Mise a jour precedente : **24 juin 2026 (soir)** — **côté DEMANDE + machine d'adoption du standard** : (1) **Serveur MCP DISTANT live** = le **connecteur universel des agents** → `https://ai-visionary.xyz/agents/mcp` (mcp-handler, Streamable HTTP, 5 outils search+cashback ; `app/agents/[transport]/route.ts`). N'importe quel bot du monde colle l'URL, zéro code. **PRIORITÉ N°1 SUIVANTE = la DISTRIBUTION** : publier le MCP sur les registres (Smithery, mcp.so, Glama, PulseMCP, registre Anthropic) + surfaces auto-descriptives (llms.txt, /for-agents) pour que les agents TROUVENT l'URL. (2) **Machine d'adoption du standard ASR autonome** : récup d'emails légère `aya/email_recover_lite.py` (~32% hit, 355k entités sans email) + 2 crons VPS (2h30 récup 2000/nuit + réimport ; 9h outreach 40/j). Outreach **élargi à TOUS les secteurs** (8291, le standard vise tout, pas juste digital). (3) **Page Pollen reframe** : « Where AI agents find the best deals » / opérateur d'agent rémunéré ; stats robustes ; abeille virée. (4) **Outreach ACTIF** (OUTREACH_ENABLED=true, premiers emails partis) + **5 offres cashback pilotes live** + boucle agent→cashback prouvée end-to-end (anti-rejeu OK). ⚠️ Incident résolu : build Turbopack cassé par playwright-poster (stubé) + rsync sans --delete → voir [[incident_vps_deploy_turbopack_stale]] (deploy front = `rm -rf .next && npm run build`). **Doctrine clé** : ASR = STANDARD OUVERT, l'outreach « faites votre ASR » est la motion FONDATRICE d'adoption ([[strategy_asr_is_the_standard]]), ne JAMAIS la couper. Voir STATE.md §Marketing.
>
> Mise a jour precedente (24 juin) : **chantier marketing (machine à deux côtés)** : (1) moteur d'**outreach automatisé** cold B2B (`lib/outreach/`, SMTP throttlé via `hello@`, List-Unsubscribe one-click, désinscription, cron warmup) — **ARMÉ mais INERTE** (`OUTREACH_ENABLED` absent = dry-run forcé) ; cible ASR = 4 456 emails digital/SaaS/fintech/crypto ; test réel reçu en inbox. (2) **pipeline partenaires cashback** : détecteur de programme d'affiliation (`lib/outreach/affiliate-detector.ts`) + table `partner_candidates` + dédup par marque + **connecteur réseaux Awin/Impact** (`lib/pollen/network-connector.ts` → `cashback_offers`). (3) **pivot copy « gratuit pour l'instant »** (FAQ/CGV FR+EN, Stripe sous le capot). (4) **Pollen Agents rendu visible** : section home `#pollen` + lien footer. Migrations 24 juin (outreach_*, partner_candidates, kind/brand). Docs : `NEOUSBOT-OUTREACH-RUNBOOK.md`, `POLLEN-DEAL-KIT.md`. **Décisions verrouillées** : Pollen = route (PAS un domaine), gratuit pour l'instant, `hello@` pour l'outreach. **Gated Cyril** : clés API Awin, filtre qualité email, GO envoi, signature des deals. Voir STATE.md §Marketing.
>
